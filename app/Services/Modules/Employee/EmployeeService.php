...
{
    public static function getBirthdateOnMonth()
    {
      $user = Auth::user();

        $employees = Employee::with(['avatar'])
            ->where('active', 1)
            ->whereMonth('birthdate', Carbon::now()->month)
            ->where(function ($query) use ($user) {
                if ($user->staff) {
                    $query->where('manager_n4_id', $user->manager_n4_id);
                }
                if (!$user->staff) {
                    $query->where('manager_n1_id', $user->manager_n1_id);
                }
            })
            ->orderBy('name')           
            ->get(['username', 'name', 'birthdate', 'active', 'uf', 'manager_n1_id', 'manager_n3_id']);
        return  $employees;
    }


    public static function getManagerEmployees($username)
    {
        return Employee::where(1, 1)
            ->where('status', 1)
            ->where(function ($query) use ($username) {
                ...
                $query->orWhere('manager_n6_id', $username);
            })
            ->get(['username', 'name']);
    }
    ...
    public static function setEmployee()
    {
        ...
        $dataInicio = Carbon::now()->subMonth(1)->startOfDay();
            ...
            LazyCollection::make($users)
                ...
                ->each(function ($chunk) {
                    ...
                    $updates = $chunk->map(function ($user) {
                        ...
                        return [
                            'id' => crc32($user['username'] . $user['date_ref']),
    ...
    public static function getManagerBySector()
    {
        // Gestores N1
        $managerN1 = Employee::with(['managerN1'])
            ->whereHas('managerN1')
            ->where('active', 1)
            ->groupBy(['sector_n1_id', 'manager_n1_id'])
            ->get([
                'sector_n1_id',
                'manager_n1_id',
            ]);

        $managerN1 = $managerN1->map(function ($item) {

            try {
                if ($item['managerN1']['active']) {
                    $temp['sector_n1_id'] = $item->sector_n1_id;
                    $temp['manager_username'] = $item->manager_n1_id;
                    $temp['hierarchical_level'] = $item['managerN1']['hierarchical_level']['id'];
                    return $temp;
                }
            } catch (\Throwable $th) {
                Log::error('caraga managar sector - m1', $item->toArray());
            }
        })->toArray();

        // Gestores N2
        $managerN2 = Employee::with(['managerN2'])
            ->whereHas('managerN2')
            ->where('active', 1)
            ->groupBy(['sector_n1_id', 'manager_n2_id'])
            ->get([
                'sector_n1_id',
                'manager_n2_id',
            ]);

        $managerN2 = $managerN2->map(function ($item) {
            try {
                if ($item['managerN2']['active']) {
                    $temp['sector_n1_id'] = $item->sector_n1_id;
                    $temp['manager_username'] = $item->manager_n2_id;
                    $temp['hierarchical_level'] = $item['managerN2']['hierarchical_level']['id'];
                    return $temp;
                }
            } catch (\Throwable $th) {
                Log::error('caraga managar sector - m2', $item->toArray());
            }
        })->toArray();

        // Gestores N3
        $managerN3 = Employee::with(['managerN3'])
            ->whereHas('managerN3')
            ->where('active', 1)
            ->groupBy(['sector_n1_id', 'manager_n3_id'])
            ->get([
                'sector_n1_id',
                'manager_n3_id',
            ]);

        $managerN3 = $managerN3->map(function ($item) {
            try {
                if ($item['managerN3']['active']) {
                    $temp['sector_n1_id'] = $item->sector_n1_id;
                    $temp['manager_username'] = $item->manager_n3_id;
                    $temp['hierarchical_level'] = $item['managerN3']['hierarchical_level']['id'];
                    return $temp;
                }
            } catch (\Throwable $th) {
                Log::error('caraga managar sector - m3', $item->toArray());
            }
        })->toArray();

        // Gestores N4
        $managerN4 = Employee::with(['managerN4'])
            ->whereHas('managerN4')
            ->where('active', 1)
            ->groupBy(['sector_n1_id', 'manager_n4_id'])
            ->get([
                'sector_n1_id',
                'manager_n4_id',
            ]);

        $managerN4 = $managerN4->map(function ($item) {
            try {
                if ($item['managerN4']['active']) {
                    $temp['sector_n1_id'] = $item->sector_n1_id;
                    $temp['manager_username'] = $item->manager_n4_id;
                    $temp['hierarchical_level'] = $item['managerN4']['hierarchical_level']['id'];
                    return $temp;
                }
            } catch (\Throwable $th) {
                Log::error('caraga managar sector - m4', $item->toArray());
            }
        })->toArray();

        // Gestores N4
        $managerN5 = Employee::with(['managerN5'])
            ->whereHas('managerN5')
            ->where('active', 1)
            ->groupBy(['sector_n1_id', 'manager_n5_id'])
            ->get([
                'sector_n1_id',
                'manager_n5_id',
            ]);

        $managerN5 = $managerN5->map(function ($item) {
            try {
                if ($item['managerN5']['active']) {
                    $temp['sector_n1_id'] = $item->sector_n1_id;
                    $temp['manager_username'] = $item->manager_n5_id;
                    $temp['hierarchical_level'] = $item['managerN5']['hierarchical_level']['id'];
                    return $temp;
                }
            } catch (\Throwable $th) {
                Log::error('caraga managar sector - m5', $item->toArray());
            }
        })->toArray();

        return array_merge($managerN1, $managerN2, $managerN3, $managerN4, $managerN5);
    }
}
