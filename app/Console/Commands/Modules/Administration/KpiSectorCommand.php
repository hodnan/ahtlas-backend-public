...
class KpiSectorCommand  extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'kpi:sector';
    ...
    public function handle()
    {
        ...
        foreach ($months as $month) {
            try {
                KpiSector::whereBetween('date_ref', [$month['start'], $month['end']])->delete();
            } catch (\Throwable $th) {
                Log::error('KpiEmployeeCommand', ['delete error', $th->getMessage()]);
            }

            foreach ($kpis as $value) {
                KpiSectorService::setKpiSectorResults($month['start'],  $value['indicator_id']);
            }
        }
