...
class UserCommand extends Command
{
  ...
  {
    // Cria um hash aleatório para ser usado como senha. 
    $hash =  env('APP_ENV', 'production') == 'dev' ? Hash::make('123456') : Hash::make(Str::uuid());
    ...
    LazyCollection::make($users)
      ->chunk(999)
      ->each(function ($chunk) use ($hash, $createdAt) {
        ...
        foreach ($updates as $user) {
          $user = (array) $user;
          try {
            User::updateOrInsert(['username' => $user['username']], $user);
          } catch (\Throwable $e) {
